...
<section id="physical-banner" class="d-flex flex-column justify-content-center align-items-center">
    ...
    <h1><strong>Physical Books</strong></h1>
        ...
        <div class="container">
            <div class="row search-sec">
                <div class=" offset-lg-1 col-lg-10 col-md-12">
                    <div class="form-row">
                        <div class="offset-lg-2 col-lg-8 col-md-12 col-sm-12 mb-5">
                            <div class="row">
      ...
      <div class="col-lg-3 col-md-4 col-sm-6 col-6 mb-3 d-flex align-items-stretch no-gutters">
        ...
        <img x-bind:src="'books/'+item.cover_photo" class="img-fluid  bk-img" alt="" />
          <div class="card-body d-flex flex-column member-info">
           ...
           <p class="card-text" x-text="'Total copies left '+item.total_stock"></p>
           <a href="#" class="readnw-btn" x-bind:href="'book-details.php?id='+item.book_id">Issue now</a>
                  </template> 
                  <template x-if="item.total_stock<=0">
                  <p class="card-text">Currently not Available</p>
                  <p class="card-text mb-4" x-text="item.availableDate!=null?'Back on : '+new Date(item.availableDate).toLocaleString():''"></p>
           <a href="#" class="readnw-btn text-white mt-auto align-self-start" x-bind:href="'reserve-book.php?id='+item.book_id">Reserve Now</a>
                  </template>  
          </div>
        </div>
      </div>
        ...
        return {
            ...
            getBook() {
                const setBooksJson = (data) => {
                    this.booksJson = JSON.parse(data).filter(book => {
                        return book.book_type === 'physical book';
                    });
                    this.booksJson.forEach(book => {
                        if (book.total_stock <= 0) {
                            $.ajax({
                                ...
                                success: function(res) {
                                    book.availableDate = JSON.parse(res)[0].return_date;
                                },
                                error: function(res) {
                                    console.log(res);
                                }
            ...
            getAllBooks() {
                ...
                const setBooksJson = (data) => {
                    var pageLen = Math.ceil(JSON.parse(data).filter(book => {
                        return book.book_type === 'physical book';
                    }).length / 50);
                    if (pageLen > 0) {
